"""
This script is used for course notes.

Author: Erick Marin
Date: 01/01/2021
"""

# Practice Quiz - Advanced Bash Concepts
#
# 1. Which command does the while loop initiate a task(s) after?
#
# [ ] done
# [x] do
# [ ] while
# [ ] n=1

# 2. Which line is correctly written to start a FOR loop with a sample.txt
# file?
#
# [x] for file in sample.txt; do
# [ ] for sample.txt do in file
# [ ] for sample.txt in file; do
# [ ] do sample.txt for file

# 3. Which of the following Bash lines contains the condition of taking an
# action when n is less than or equal to 9?
#
# [x] while [ $n -le 9 ]; dowhile [ $n -le 9 ]; do
# [ ] while [ $n -lt 9 ]; do
# [ ] while [ $n -ge 9 ]; do
# [ ] while [ $n -ot 9 ]; do

# 4. Which of the following statements are true regarding Bash and Python?
# [Check all that apply]
#
# [x] Complex scripts are better suited to Python.
# [ ] Bash scripts work on all platforms.
# [x] Python can more easily operate on strings, lists, and dictionaries.
# [x] If a script requires testing, Python is preferable.

# 5. The _____ command lets us take only bits of each line using a field
# delimiter.
#
# [x] cut
# [ ] echo
# [ ] mv
# [ ] sleep
